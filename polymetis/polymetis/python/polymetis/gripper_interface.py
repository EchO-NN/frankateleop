...
class GripperInterface:
    ...
    def _send_gripper_command(self, command, msg, blocking: bool = True) -> None:
        ...
        # Keep only the newest queued command so teleop loops never block behind
        # stale gripper targets while a previous command is still executing.
        try:
            while True:
        ...
        try:
            self._command_queue.put_nowait((command, msg))
        except queue.Full:
            log.debug("Dropping stale non-blocking gripper command because queue is full.")
