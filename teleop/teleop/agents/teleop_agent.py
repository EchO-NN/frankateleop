...
#left fr3
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTB624NR-if00-port0": DynamixelRobotConfig(
...
#right_fr3
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTAUMOPA-if00-port0": DynamixelRobotConfig(
        ...
        joint_signs=(1, 1, 1, 1, 1, 1, 1),
        gripper_config=(8, 190.0, 155.0),
    ),
    # right_fr3 (replacement USB-serial adapter)
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTBGPXIY-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6, 7),
        joint_offsets=(
            2 * np.pi / 2,
            2 * np.pi / 2,
